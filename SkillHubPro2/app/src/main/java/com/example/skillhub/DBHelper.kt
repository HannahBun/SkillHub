...
class DBHelper(context: Context) : SQLiteOpenHelper(context, "skillhub.db", null, 2) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL(
            // added instructor_id column (integer)
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        // Simple upgrade strategy: drop and recreate. For production use migrations.
    ...
    // For compatibility with earlier code (simple signature) - fallback uses instructorId = -1
    fun insertCourse(title: String, description: String) : Long {
        return addCourse(title, description, 0.0, -1)
    }
    ...
    fun getCourseById(courseId: Int): Course? {
        ...
        val course = if (cursor.moveToFirst()) {
            Course(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getDouble(3), cursor.getInt(4))
        } else null
        cursor.close()
        return course
    }
